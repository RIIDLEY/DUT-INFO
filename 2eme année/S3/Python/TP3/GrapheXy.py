...
def Graphe_Des_or(G):
	...
	A=GrapheZero(n) #nouveau graphe
	i=0
	while(i<n) :
		j=i+1
		while(j<n) :
			if(FracComp(G[i][j],G[j][i])==0) :		#comparaison valuation des arc
				A[i][j]=G[i][j]						#copie sur le nouveau graph
				A[j][i]=G[i][j]
			elif(FracComp(G[i][j],G[j][i])==1) :
				A[i][j]=G[i][j]
				A[j][i]=G[i][j]
			elif(FracComp(G[i][j],G[j][i])==-1) :
				A[i][j]=G[j][i]
...
def K_or(n) :			#K= tout est relié					#http://www.ataraxy.info/Graphes?Chapitres/Generalites.graphesstandards
	G=GrapheZero(n)
	i=0
	while i<n :
		j=0
...
def existeCircuitEulerien_or(G) :
	i = 0
	while i<n:
		if demi_deg_ext(i,G)!=demi_deg_int(i,G) :
			return False
		i+=1
	return True
...
def existeChaineEulerien_or(G) :
	...
	i=0
	while i < n:
		if (demi_deg_ext(i,G) == demi_deg_int(i,G)+1 or demi_deg_ext(i,G) == demi_deg_int(i,G)-1):
			nb+=1
		elif (demi_deg_ext(i,G) == demi_deg_int(i,G)):
			nb1+=1
		i+=1
	if nb == 2 and nb1 == len(G)-2 :
		return True
	return False
...
print(GrapheAff(K_or(5)))
print(GrapheAff(C_or(6)))
print(GrapheAff(Z_or(6)))
print(existeCircuitEulerien_or(G1))
print(existeCircuitEulerien_or(Z_or(5)))
print(existeChemin(G1, 1, 0, n-1))
print(existeChaineEulerien_non_or(Graphe_Des_or(G1)))
print(GrapheAff(Graphe_Des_or(C_or(6))))
print(existeChaineEulerien_non_or(Graphe_Des_or(C_or(6))))
print(existeChaineEulerien_or(C_or(6)))
