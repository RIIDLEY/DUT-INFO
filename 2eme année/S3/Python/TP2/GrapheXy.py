...
def existeChemin(G, l, a, b) :
	...
	while i < n :
		j = 0
		while j < n :
			print("longueur : " + str(longueur) + " l :" + str(l) + " j : "
					+ str(j) + " i: " + str(i) + " b : " + str(b))
			if G[i][j] != 0:
				longueur = longueur + G[i][j]
				if longueur == l and j == b :
					return True
				elif longueur > l :
					return False

				else:
					if existeChemin(G,l-longueur,j,b):
						return True
			j = j + 1
		i = i + 1
